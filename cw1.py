from bucket_fill import show_image, stringify_image, load_image

def valid_seed_type(seed_coords):
    """
    Asks for user input for the initial seed coordinate.

    If the seed coordinate do not correspond to a
    tuple -> (int x, int y), the function will result
    in an error and a request for only integer inputs.
    """
    x, y = seed_coords
...
def include_neighbours(coords, past_set, future_set, img):
    ...
    Second: Set of tuples corresponding to already checked sites -> {(x,y)}
        Will be used so that sites that have already been checked
        do not need to be checked again, and therefore their
...
def iterate_future_set(original_image, future_set, past_set):
    ...
    -----------
    Does not return a result. This is run in a while loop until
    the past_set and future_set are equal (i.e. all valid sites
    to check have been checked)
    """
...
def fill(image, seed_coords):
    ...
    if not valid_seed(seed_coords, image):
        return image
    ...
    future_set = extend_future_set(seed_coords, image,
                                future_set, past_set)
